 ...
 * not yet implemented.)
 * Advances FILE's position by the number of bytes read. */
/* 파일의 현재 위치에서 시작하여 버퍼에서 파일로 크기 바이트를 씁니다.
	실제로 쓴 바이트 수를 반환합니다. 파일 끝에 도달한 경우 SIZE보다 작을 수 있습니다.
	(일반적으로 이러한 경우에는 파일을 확장하지만, 파일 확장은 아직 구현되지 않았습니다.)
읽은 바이트 수만큼 파일의 위치를 전진시킵니다.*/
off_t
